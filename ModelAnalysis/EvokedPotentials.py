"""
Here, the evoked potentials for in and outside the focus of attention are obtained for the different attentional mechanisms and the neutral model.
...
if compute_cues == True:
    ...
    Ax = np.zeros((len(x), 2))
    #Ax_test = np.zeros((len(test_index), 2))
...
if test_locations == True:
    ...
    plt.ylim([0, 1])
    plt.legend()
    plt.savefig('/mnt/Googleplex2/PycharmProjects/SpatialAttention_asn/Misc/CueLocations.png')
...
if test_images == True:
    ...
    fig.savefig('/mnt/Googleplex2/PycharmProjects/SpatialAttention_asn/Misc/TESTImage.png')
...
#%% Params
layer_idx = [25]
conds = ['P-0.45_I-0_O-0']#,'P-0_I-0_O-0.3','P-0_I-0.15_O-0'
    ...
    for l in layer_idx:
        ...
        if cond == 'neutral':
            ...
            if compute_dataset == True:
                ...
                evoked = np.full((len(x_test), time_steps), np.nan)
                evoked_all = np.full((len(x_test), time_steps), np.nan)
