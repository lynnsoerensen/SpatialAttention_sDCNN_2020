...
if test_locations == True:
    ...
    plt.ylim([0, 1])
    plt.legend()
    plt.savefig(dir_path + '/CueLocations.png')
...
if compute_averageBaseline == True:
    img_dir_train = dir_path + '/Datasets/' + dataset + '/images/train2017_multi_radial/'

    dataset_base = load_pickle(
        dir_path + '/Datasets/' + dataset + '/train2017_multi_radial.pickle')
...
#%%
tracker_path = dir_path + '/ModelAnalysis/'
tracker_name = 'FiringRates_readout.pkl'
if new_tracker == True:
    tracker = {}
...
conds = ['P-0.45_I-0_O-0']#, 'P-0_I-0.15_O-0','P-0_I-0_O-0.3'
...
for cond in conds:
    ...
    for l in layer_idx:
        ...
        s = 16  # should be even.
        samples = np.arange(0, len(x_test), s)

        if cond == 'neutral':
            if cond not in tracker[l]:
                tracker[l][cond] = {}

            readOut = ASN_2D(filters=spikingModel.layers[l].output.shape[-1].value, last_layer=False, mf=mf,
            ...
            if compute_grayBaseline == True:
                ...
                grayImage = np.ones(x_test[:1].shape) * 0.5
                empty_y = np.zeros(y_test[:1].shape)
                ...
                tracker[l][cond]['MUA_baseline'] = np.sum(
                        baselinePredictions[0, :,:, :, :] > 0, axis=-1)
            ...
            if compute_averageBaseline == True:
                ...
                empty_y = np.zeros((avg_imgs.shape[0], y_test.shape[1]))
                ...
                tracker[l][cond]['MUA_baseline_average'] = np.sum(
            ...
            if compute_dataset == True:
                ...
                mua = np.full((len(x_test), time_steps), np.nan)
                mua_all = np.full((len(x_test), time_steps), np.nan)
                ...
                for n in range(len(samples)):
                    print(n)
                    if n == len(samples) - 1:
                        idx = np.arange(samples[n], len(x_test))
                    else:
                        idx = np.arange(samples[n], samples[n + 1])

                    spikingPredictions = predict_generator(model_shortened, x_test[idx],
                                                           y_test[idx],
                                                           time_steps,
                                                           batch_size,
                                                           generator=TimeReplication_onset,
                    ...
                    for id_n in range(len(idx)):
                        ...
                        mua[idx[id_n]] = np.sum(neurons > 0, axis=1)  # sum over feature maps
                        mua_all[idx[id_n]] = np.sum(
                            np.sum(np.sum(spikingPredictions[id_n] > 0, axis=1), axis=1),
                            axis=1)  # sum over all neurons in this layer

                    del spikingPredictions

                # Write it back to the tracker
                tracker[l][cond]['MUA'] = mua
                tracker[l][cond]['MUA_all'] = mua_all
            ...
            readOut = ASN_2D(filters=layer_interest.output.shape[-1].value, last_layer=False, mf=mf,
                             kernel_initializer='ones',
            ...
            for att in ['valid', 'invalid']:
                ...
                if compute_averageBaseline == True:
                    ...
                    if att == 'valid':
                        # place the kernel in the center
                        att_loc_baseline = np.array([0.5, 0.5])[np.newaxis, :] * np.ones((avg_imgs.shape[0],1))
                    elif att == 'invalid':
                        # place the kernel in the periphery
                        att_loc_baseline = np.array([0.1, 0.2])[np.newaxis, :]* np.ones((avg_imgs.shape[0],1))
                    else:
                        raise ValueError('This cue condition is not implemented.')
                    ...
                    tracker[l][cond][att]['MUA_baseline_average'] = np.sum(
                        baselinePredictions > 0, axis=-1)
                ...
                if compute_dataset == True:
                    ...
                    # Write it back to the tracker
                    tracker[l][cond][att]['MUA'] = mua
                    tracker[l][cond][att]['MUA_all'] = mua_all
