...
spike_test =False
count_units = False
test_attn_params = True
make_split = False
mechanism = 'inputGain'#'interactionsPrecision'  #'inputGainPrecision' 'outputGain' 'outputGainPrecision' 'interactions' 'interactionsPrecision'
# %%
...
# %% load in the dataset:
img_dir_test = '/mnt/Googolplex/PycharmProjects/SpatialAttention_sDCNN_2020/Datasets/' + dataset + '/images/val2017_single_radial/'
...
features = load_pickle(
    '/mnt/Googolplex/PycharmProjects/SpatialAttention_sDCNN_2020/Datasets/' + dataset + '/Features_single_radial.pickle')
...
lr = 1e-4
weight_path = '/mnt/Googolplex/PycharmProjects/SpatialAttention_sDCNN_2020/ModelTraining/' + dataset + '/weights_Adam_constant_0.0001.h5'
model_path = '/mnt/Googolplex/PycharmProjects/SpatialAttention_sDCNN_2020/ModelTraining/' + dataset + "/model.json"
...
tracker_path = '/mnt/Googolplex/PycharmProjects/SpatialAttention_sDCNN_2020/ModelEvaluation/'
if new_tracker == True:
    tracker = {}
...
if compute_metrics == True:
    ...
    tracker['analog']['predictions'] = preds
    tracker['analog']['binary_acc'] = scores[1]
    tracker['analog']['acc'] = scores[2]
    tracker['analog']['accTop2'] = scores[3]
    tracker['analog']['accTop3'] = scores[4]

    tracker['analog']['F1'] = f1_score(y_train, preds >= 0.5,
                                       average='weighted')

    tracker['analog']['AUC'] = roc_auc_score(y_train, preds)
    ...
    tracker['analog']['tp_rate'] = tp_rate(preds >= 0.5, y_train)
    tracker['analog']['fp_rate'] = fp_rate(preds >= 0.5, y_train)
    ...
    tracker['analog']['chance_acc'] = np.percentile(permutation_scores[0, :], 95)
    tracker['analog']['chance_accTop2'] = np.percentile(permutation_scores[1, :], 95)
    tracker['analog']['chance_accTop3'] = np.percentile(permutation_scores[2, :], 95)
    tracker['analog']['chance_F1'] = np.percentile(permutation_scores[3, :], 95)
    tracker['analog']['chance_AUC'] = np.percentile(permutation_scores[4, :], 95)
...
if spike_test == True:
    ...
    for mf in mfs:
        if ("mf_" + str(mf) not in tracker):
            ...
            tracker['mf_' + str(mf)]['AUC'] = roc_auc_score(tracker['y_train_single'],
...
if test_attn_params == True:
    ...
    if mechanism == 'inputGain':
        ...
        gains_input = [0, 0.05,  0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
        gains_output = [0]
    elif mechanism == 'inputGainPrecision':
        precision_modes = [True]
        gains_input = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
        gains_output = [0]
    elif mechanism == 'outputGain':
        precision_modes = [False]
        gains_output = [0, 0.05,  0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
        gains_input = [0]
    elif mechanism == 'outputGainPrecision':
        precision_modes = [True]
        gains_output = [0, 0.05,  0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
        gains_input = [0]
    elif mechanism == 'interactions':
        precision_modes = [False]
        gains_input = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
        gains_output = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
    elif mechanism == 'interactionsPrecision':
        precision_modes = [True]
        gains_input = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
        gains_output = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
    else:
    ...
    for gain_output in gains_output:
        for gain_input in gains_input:
            for precision_mode in precision_modes:
                ...
                cond = 'P-' + str(int(precision_mode)) + '_I-' + str(gain_input) + '_O-' + str(gain_output)
                ...
                if ('valid' not in tracker['attention_' + str(mf)][cond]) | (
                                'invalid' not in tracker['attention_' + str(mf)][cond]):
                    ...
                    for att_cond in ['valid', 'invalid']:
                        ...
                        if att_cond not in tracker['attention_' + str(mf)][cond]:
                            ...
                            tracker['attention_' + str(mf)][cond][att_cond]["accTop2"] = evaluate_predictions(predictions[0],
                                                                                                          y_train,
                                                                                                          start_eval,
                                                                                                          time_steps, 2,
                                                                                                          num_targets=1)
                            tracker['attention_' + str(mf)][cond][att_cond]["accTop3"] = evaluate_predictions(predictions[0],
                                                                                                          y_train,
                                                                                                          start_eval,
                                                                                                          time_steps, 3,
                                                                                                          num_targets=1)
                            ...
                            tracker['attention_' + str(mf)][cond][att_cond]['AUC'] = roc_auc_score(tracker['y_train_single'],
