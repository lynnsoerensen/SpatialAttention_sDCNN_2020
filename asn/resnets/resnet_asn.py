...
from asn.layers.training import ASNTransfer as Activation
...
def _bn_relu(input):
    ...
    #return Activation(activation='relu')(norm)
...
def _shortcut_v0(input, residual):
    """Adds a shortcut between input and residual block and merges them with "sum"
    """
    # Expand channels of shortcut to match residual.
    # Stride appropriately to match residual (width, height)
    ...
    added = Add()([shortcut, residual])
    #return Activation(activation='relu')(added)
...
def basic_block_v0(nb_filter, init_subsample=(1, 1), is_first_block_of_first_layer=False):
    """Basic 3 X 3 convolution blocks for use on resnets with layers <= 34.
    Based on first ResNet generation (without preactivation)
    """
    def f(input):

        conv1 = _conv_bn_relu(nb_filter=nb_filter, nb_row=3, nb_col=3,
                              subsample=init_subsample)(input)

        residual = _conv_bn(nb_filter=nb_filter, nb_row=3, nb_col=3)(conv1)
        return _shortcut_v0(input, residual)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        if input_shape[0] == 32: # for CIFAR
            nb_filter = 16
            pool1 = _conv_bn_relu(nb_filter=nb_filter, nb_row=3, nb_col=3, subsample=(1, 1))(asninput)

            repetitions = repetitions[:-1]
        else:
            nb_filter = 64
            conv1 = _conv_bn_relu(nb_filter=nb_filter, nb_row=7, nb_col=7, subsample=(2, 2))(asninput)
    ...
    @staticmethod
    def build_resnet_18_v0(input_shape, num_outputs):
        return ResnetBuilder.build(input_shape, num_outputs, basic_block_v0, [2, 2, 2, 2])

    @staticmethod
    def build_resnet_20_CIFAR(input_shape, num_outputs):
        return ResnetBuilder.build(input_shape, num_outputs, basic_block, [2, 2, 2, 2])
